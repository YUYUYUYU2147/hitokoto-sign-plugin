...
const defaultConfig = {
  ...
  hitokotoApi: 'http://113.31.103.19:8848',
  hitokotoType: '',
  signPrefix: '',
...
// 全局单例：确保定时任务只初始化一次，防止 TRSS-Yunzai 多次实例化导致重复任务
...
export class HitokotoSignPlugin extends plugin {
  constructor() {
    ...
    // 只初始化一次定时任务（扫描配置文件恢复所有 Bot 的定时任务）
    if (!_globalInitDone) {
  ...
  _initSchedules() {
    try {
      ...
      for (const f of files) {
        ...
        // 跳过已存在任务的 Bot，防止重复创建
        if (this._jobs[selfQq]?.signJob || this._jobs[selfQq]?.shuoshuoJob) continue
        const cfg = loadConfig(selfQq)
        if (cfg.enableSignUpdate || cfg.enableShuoshuoUpdate) {
          this.refreshSchedule(selfQq, cfg)
          console.log(`[一言][${selfQq}] 已恢复定时任务 | 签名:${cfg.signCron} 说说:${cfg.shuoshuoCron}`)
          restored++
        }
      }
      if (restored > 0) console.log(`[一言] 共恢复 ${restored} 个Bot的定时任务`)
    } catch (e) {
      console.error('[一言] 初始化调度失败', e)
    }
  }
